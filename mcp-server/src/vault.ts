...
export async function appendNote(relativePath: string, content: string) {
  const fullPath = path.join(VAULT_PATH, relativePath);
  await ensureDir(path.dirname(fullPath));
...
export async function listNotes(subfolder: string = ''): Promise<string[]> {
...
export async function listDirectories(subfolder: string = ''): Promise<string[]> {
  const dirPath = path.join(VAULT_PATH, subfolder);
  try {
...
export async function searchNotes(query: string): Promise<Array<{ path: string; excerpt: string }>> {
